...
function BadgePreview({ variant, children }: { variant?: string; children?: React.ReactNode }) {
  const colors: Record<string, string> = {
    default: "bg-surface-700 text-surface-200",
  };
  return (
    <span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium", colors[variant || "default"])}>
      {children || "Badge"}
    </span>
  );
}

function ButtonPreview({ variant, size, fullWidth, disabled, children }: { variant?: string; size?: string; fullWidth?: boolean; disabled?: boolean; children?: React.ReactNode }) {
...
function CardPreview({ variant, children }: { variant?: string; children?: React.ReactNode }) {
  const variants: Record<string, string> = {
    default: "bg-surface-800 border-border",
    outlined: "border-2 border-surface-700 bg-transparent",
    elevated: "bg-surface-800 border-border shadow-lg shadow-black/20",
    ghost: "bg-transparent border-transparent",
  };
  return (
    <div className={cn("rounded-xl border overflow-hidden", variants[variant || "default"])}>
      {children || (
...
function CardHeaderPreview({ children }: { children?: React.ReactNode }) {
  return (
...
function CardBodyPreview({ children }: { children?: React.ReactNode }) {
  return <div className="px-4 py-3 text-surface-300 text-sm space-y-2">{children || "Card body content"}</div>;
}

function CardFooterPreview({ children }: { children?: React.ReactNode }) {
  return <div className="px-4 py-3 border-t border-surface-700 bg-surface-800/50">{children || <span className="text-surface-400 text-xs">Card Footer</span>}</div>;
}



function AlertPreview({ variant, title, children }: { variant?: string; title?: string; children?: React.ReactNode }) {
  const colors: Record<string, string> = {
  ...
  return (
    <div className={cn("rounded-lg border px-4 py-3 text-sm", colors[variant || "info"])}>
...
function InputPreview({ label, placeholder, error }: { label?: string; placeholder?: string; error?: boolean }) {
  return (
    <div className="space-y-1.5">
      {label && <label className="text-xs font-medium text-surface-300">{label}</label>}
      <div className={cn("w-full px-3 py-2 rounded-lg border text-sm bg-surface-900 text-surface-400", error ? "border-red-500" : "border-surface-700")}>
...
function SelectPreview({ label, placeholder }: { label?: string; placeholder?: string }) {
  return (
    <div className="space-y-1.5">
      {label && <label className="text-xs font-medium text-surface-300">{label}</label>}
...
function TextareaPreview({ label, placeholder }: { label?: string; placeholder?: string }) {
  return (
    <div className="space-y-1.5">
      {label && <label className="text-xs font-medium text-surface-300">{label}</label>}
...
function AvatarPreview({ fallback, size }: { src?: string; alt?: string; fallback?: string; size?: string }) {
  const sizes: Record<string, string> = { sm: "w-6 h-6 text-[10px]", md: "w-8 h-8 text-xs", lg: "w-12 h-12 text-sm" };
  return (
    <div className={cn("rounded-full bg-surface-700 flex items-center justify-center text-surface-400 font-medium", sizes[size || "md"])}>
      {fallback || "A"}
    </div>
  );
}
...
function LoadingPreview({ size, label }: { variant?: string; size?: string; label?: string }) {
  ...
  return (
    <div className="flex items-center gap-2 text-surface-400">
      <div className={cn("animate-spin rounded-full border-2 border-surface-600 border-t-awesome-500", sizes[size || "md"])} />
...
function TablePreview({ columns, rows }: { columns?: { key: string; label: string }[]; rows?: Record<string, string>[] }) {
  const cols = columns || [{ key: "a", label: "Column 1" }, { key: "b", label: "Column 2" }];
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-surface-800">
            {cols.map((col) => (
              <th key={col.key} className="px-3 py-2 text-left text-xs font-medium text-surface-400">{col.label}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {data.map((row, i) => (
            <tr key={i} className="border-t border-surface-800">
              {cols.map((col) => (
                <td key={col.key} className="px-3 py-2 text-surface-300">{row[col.key]}</td>
              ))}
            </tr>
          ))}
        </tbody>
    ...
    <div>
      <div className="flex gap-1 border-b border-surface-700 mb-3">
        {["Tab 1", "Tab 2", "Tab 3"].map((tab) => (
          <button key={tab} className={cn("px-3 py-1.5 text-xs font-medium rounded-t-lg", tab === "Tab 1" ? "text-awesome-400 border-b-2 border-awesome-500" : "text-surface-400")}>
            {tab}
          </button>
        ))}
      </div>
...
function PaginationPreview({ currentPage }: { currentPage?: number }) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      {[1, 2, 3].map((p) => (
        <span key={p} className={cn("px-2 py-1 text-xs rounded", p === page ? "bg-awesome-500 text-white" : "text-surface-400")}>{p}</span>
      ))}
      <span className="px-2 py-1 text-xs text-surface-400">›</span>
    </div>
  );
...
function TooltipPreview({ children }: { content?: string; children?: React.ReactNode }) {
  return (
    <div className="relative inline-flex">
      <span className="text-sm text-surface-400 border-b border-dashed border-surface-600 cursor-help">{children || "Hover me"}</span>
    </div>
  );
}
  ...
  return (
    <div className="w-48 rounded-lg border border-surface-700 bg-surface-900 p-2 space-y-0.5">
      {["Dashboard", "Analytics", "Settings", "Help"].map((item) => (
        <div key={item} className="px-2 py-1.5 text-xs text-surface-300 rounded hover:bg-surface-800 cursor-pointer">
          {item}
        </div>
      ))}
      {children}
  ...
  return (
    <div className="flex items-center gap-1 px-2 py-1.5 rounded-lg border border-surface-700 bg-surface-900">
      {["File", "Edit", "View", "Help"].map((item) => (
        <span key={item} className="px-2 py-1 text-xs text-surface-300 rounded hover:bg-surface-800 cursor-pointer">{item}</span>
      ))}
    </div>
  );
}
...
function ToastPreview({ variant, title }: { variant?: string; title?: string }) {
  ...
  return (
    <div className={cn("rounded-lg border px-3 py-2 text-sm flex items-center justify-between max-w-xs", colors[variant || "default"])}>
...
function FormPreview({ children }: { children?: React.ReactNode }) {
  return <div className="space-y-3 p-4 rounded-lg border border-surface-700 bg-surface-900/50">{children || <><InputPreview label="Email" /><ButtonPreview variant="primary">Submit</ButtonPreview></>}</div>;
}
...
function HeadingPreview({ size, children, className }: { size?: string; children?: React.ReactNode; className?: string }) {
  const sizes: Record<string, string> = {
    xs: "text-sm",
    sm: "text-base",
  ...
  return <div className={cn("text-surface-100 font-semibold", sizes[size || "md"], className)}>{children}</div>;
}

export const previewComponents: Record<string, React.ComponentType<any>> = {
  section: ({ children, className, ...props }) => <section className={cn("py-8", className)} {...props}>{children}</section>,
  div: ({ children, className, ...props }) => <div className={cn(className)} {...props}>{children}</div>,
  header: ({ children, className, ...props }) => <header className={cn(className)} {...props}>{children}</header>,
  nav: ({ children, className, ...props }) => <nav className={cn(className)} {...props}>{children}</nav>,
  main: ({ children, className, ...props }) => <main className={cn(className)} {...props}>{children}</main>,
  footer: ({ children, className, ...props }) => <footer className={cn(className)} {...props}>{children}</footer>,
  article: ({ children, className, ...props }) => <article className={cn(className)} {...props}>{children}</article>,
  aside: ({ children, className, ...props }) => <aside className={cn(className)} {...props}>{children}</aside>,
  h1: ({ children, className, ...props }) => <h1 className={cn("text-2xl font-bold text-surface-100", className)} {...props}>{children}</h1>,
  h2: ({ children, className, ...props }) => <h2 className={cn("text-xl font-bold text-surface-100", className)} {...props}>{children}</h2>,
  h3: ({ children, className, ...props }) => <h3 className={cn("text-lg font-semibold text-surface-200", className)} {...props}>{children}</h3>,
  h4: ({ children, className, ...props }) => <h4 className={cn("text-base font-medium text-surface-200", className)} {...props}>{children}</h4>,
  p: ({ children, className, ...props }) => <p className={cn("text-sm text-surface-300", className)} {...props}>{children}</p>,
  span: ({ children, className, ...props }) => <span className={cn(className)} {...props}>{children}</span>,
  ul: ({ children, className, ...props }) => <ul className={cn("space-y-1 list-disc list-inside text-surface-300 text-sm", className)} {...props}>{children}</ul>,
  li: ({ children, className, ...props }) => <li className={cn("text-surface-300 text-sm", className)} {...props}>{children}</li>,
  img: () => <div className="w-full h-32 bg-surface-700 rounded-lg flex items-center justify-center text-surface-500 text-xs">Image</div>,
  a: ({ children, className, ...props }) => <a className={cn("text-awesome-400 hover:underline cursor-pointer", className)} {...props}>{children}</a>,
  ...
  Toast: ({ children, ...props }) => <ToastPreview {...props}>{children}</ToastPreview>,
   ...
   "Accordion.Item": ({ children }) => <div className="px-3 py-2 border-b border-surface-700 text-sm">{children}</div>,
   "DropdownMenu.Trigger": ({ children }) => <div className="inline-block">{children}</div>,
   "DropdownMenu.Content": ({ children }) => <div className="p-2 bg-surface-800 rounded-lg border border-surface-700">{children}</div>,
   "DropdownMenu.Item": ({ children }) => <div className="px-3 py-1.5 text-sm text-surface-200 hover:bg-surface-700 rounded cursor-pointer">{children}</div>,
   "Dialog.Content": ({ children }) => <div className="p-6 bg-surface-800 rounded-xl border border-surface-700">{children}</div>,
