...
var ReactNativeTranspiler = class extends transpilerShared.BaseTranspiler {
  ...
  generateElement(node, styles, depth) {
    ...
    if (!children || children.length === 0) {
      return transpilerShared.indent(`<${rnTag}${attrs} />`, depth);
    }
    ...
    if (shouldWrapInText) {
      return [
        transpilerShared.indent(`<${rnTag}${attrs}>`, depth),
        transpilerShared.indent(`<Text>${childrenJSX}</Text>`, depth + 1),
        transpilerShared.indent(`</${rnTag}>`, depth)
      ].join("\n");
    }
    return [
      transpilerShared.indent(`<${rnTag}${attrs}>`, depth),
  ...
  generateText(text, depth) {
    ...
    if (segments.length === 1 && segments[0]?.type === "expression") {
      return transpilerShared.indent(`{${this.convertExprToRN(segments[0].value)}}`, depth);
    }
  ...
  generateSlot(node, depth) {
    ...
    if (node.fallback) {
      return transpilerShared.indent(`{${propName} ?? '${node.fallback}'}`, depth);
    }
...
${transpilerShared.indent(elseJSX, 1)}
)}`, depth);
    }
    return transpilerShared.indent(`{${condition} && (
${transpilerShared.indent(thenJSX, 1)}
...
${transpilerShared.indent(`<React.Fragment key={${keyExpr}}>`, depth + 1)}
${childrenJSX}
${transpilerShared.indent("</React.Fragment>", depth + 1)}
${transpilerShared.indent(")", depth)}))}`,
      depth
    );
  }
  generateComponentRef(node, depth) {
    ...
    const propsStr = Object.entries(props).map(([key, value]) => {
      if (transpilerShared.isPureExpression(value)) {
        return `${key}={${this.convertExprToRN(transpilerShared.extractExpression(value))}}`;
      }
      return `${key}="${value}"`;
    }).join(" ");
  ...
  buildRNAttributes(node, styles) {
    ...
    if (node.attributes) {
      for (const [key, value] of Object.entries(node.attributes)) {
        ...
        if (!rnKey) continue;
        if (transpilerShared.isPureExpression(value)) {
          parts.push(`${rnKey}={${this.convertExprToRN(transpilerShared.extractExpression(value))}}`);
    ...
    if (node.style) {
      const inlineStyle = Object.entries(node.style).map(([prop, val]) => {
        const camelProp = transpilerShared.toCamelCase(prop);
        if (transpilerShared.isPureExpression(val)) {
          return `${camelProp}: ${this.convertExprToRN(transpilerShared.extractExpression(val))}`;
        }
  ...
  resolveStyleKeys(node, _styles) {
    const keys = [];
    if (node.class) {
      const segments = transpilerShared.parseExpression(node.class);
...
exports.ReactNativeTranspiler = ReactNativeTranspiler;
