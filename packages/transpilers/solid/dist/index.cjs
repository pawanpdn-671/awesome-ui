...
var SolidTranspiler = class extends transpilerShared.BaseTranspiler {
  framework = "solid";
  fileExtension = ".tsx";
  language = "typescript";
  ...
  generateText(text, depth) {
    ...
    if (segments.length === 1 && segments[0]?.type === "expression") {
      return transpilerShared.indent(`{${this.convertExprToSolid(segments[0].value)}}`, depth);
    }
  ...
  generateConditional(node, styles, depth) {
    ...
    if (node.else) {
      return [
        transpilerShared.indent(`<Switch>`, depth),
        transpilerShared.indent(`<Match when={${condition}}>`, depth + 1),
        transpilerShared.indent(thenContent, depth + 2),
        transpilerShared.indent(`</Match>`, depth + 1),
        transpilerShared.indent(`<Match when={!(${condition})}>`, depth + 1),
        transpilerShared.indent(elseContent, depth + 2),
        transpilerShared.indent(`</Match>`, depth + 1),
        transpilerShared.indent(`</Switch>`, depth)
      ].join("\n");
    }
    return transpilerShared.indent(`<Show when={${condition}}>${thenContent}</Show>`, depth);
  }
  ...
  generateComponentRef(node, depth) {
    ...
    const propsStr = Object.entries(props).map(([key, value]) => {
      if (transpilerShared.isPureExpression(value)) {
        return `${key}={${this.convertExprToSolid(transpilerShared.extractExpression(value))}}`;
      }
      return `${key}="${value}"`;
    }).join(" ");
  ...
  buildSolidAttributes(node) {
    ...
    if (node.attributes) {
      for (const [key, value] of Object.entries(node.attributes)) {
        ...
        if (transpilerShared.isPureExpression(value)) {
          const expr = this.convertExprToSolid(transpilerShared.extractExpression(value));
    ...
    if (node.style) {
      const styleEntries = Object.entries(node.style).map(([prop, val]) => {
        ...
        if (transpilerShared.isPureExpression(val)) {
          return `${camelProp}: ${this.convertExprToSolid(transpilerShared.extractExpression(val))}`;
        }
...
exports.SolidTranspiler = SolidTranspiler;
