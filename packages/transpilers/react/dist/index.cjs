...
function generateText(text, depth) {
  ...
  if (segments.length === 1 && segments[0]?.type === "expression") {
    return transpilerShared.indent(`{${convertExprToReact(segments[0].value)}}`, depth);
  }
...
function generateComponentRef(node, depth) {
  ...
  const propsStr = Object.entries(props).map(([key, value]) => {
    if (transpilerShared.isPureExpression(value)) {
      return `${key}={${convertExprToReact(transpilerShared.extractExpression(value))}}`;
    }
    return `${key}="${value}"`;
  }).join(" ");
...
function buildAttributes(node, _styles, isRoot) {
  ...
  if (node.attributes) {
    for (const [key, value] of Object.entries(node.attributes)) {
      ...
      if (transpilerShared.isPureExpression(value)) {
        parts.push(`${reactKey}={${convertExprToReact(transpilerShared.extractExpression(value))}}`);
      } else {
  ...
  if (node.style) {
    const styleEntries = Object.entries(node.style).map(([prop, val]) => {
      ...
      if (transpilerShared.isPureExpression(val)) {
        return `${camelProp}: ${convertExprToReact(transpilerShared.extractExpression(val))}`;
      }
...
// src/react-transpiler.ts
var ReactTranspiler = class extends transpilerShared.BaseTranspiler {
  framework = "react";
  fileExtension = ".tsx";
  language = "typescript";
  /**
...
exports.ReactTranspiler = ReactTranspiler;
exports.generateJSX = generateJSX;
exports.generatePropsDestructure = generatePropsDestructure;
