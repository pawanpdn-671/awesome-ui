...
var VueTranspiler = class extends transpilerShared.BaseTranspiler {
  framework = "vue";
  fileExtension = ".vue";
  language = "vue";
  /**
  ...
  generateSlot(node, depth) {
    ...
    if (slotName === "default") {
      if (node.fallback) {
        return [
          transpilerShared.indent("<slot>", depth),
          transpilerShared.indent(node.fallback, depth + 1),
          transpilerShared.indent("</slot>", depth)
        ].join("\n");
      }
      return transpilerShared.indent("<slot />", depth);
    }
    if (node.fallback) {
      return [
        transpilerShared.indent(`<slot name="${slotName}">`, depth),
        transpilerShared.indent(node.fallback, depth + 1),
        transpilerShared.indent("</slot>", depth)
      ].join("\n");
    }
    return transpilerShared.indent(`<slot name="${slotName}" />`, depth);
  }
  /**
  ...
  generateConditional(node, styles, depth) {
    ...
    if (thenNode.tag) {
      ...
      let thenHTML2;
      if (!children || children.length === 0) {
        thenHTML2 = transpilerShared.indent(`<${thenNode.tag}${vIfAttr}${thenAttrs} />`, depth);
      } else {
        const childrenHTML = children.map((child) => this.generateNode(child, styles, depth + 1)).join("\n");
        thenHTML2 = [
          transpilerShared.indent(`<${thenNode.tag}${vIfAttr}${thenAttrs}>`, depth),
          childrenHTML,
          transpilerShared.indent(`</${thenNode.tag}>`, depth)
        ].join("\n");
      }
      if (node.else) {
    ...
    let result = [
      transpilerShared.indent(`<template v-if="${condition}">`, depth),
      thenHTML,
      transpilerShared.indent("</template>", depth)
    ].join("\n");
    if (node.else) {
      const elseHTML = this.generateNode(node.else, styles, depth + 1);
      result += "\n" + [
        transpilerShared.indent("<template v-else>", depth),
        elseHTML,
        transpilerShared.indent("</template>", depth)
      ].join("\n");
    }
  ...
  generateNodeWithDirective(node, styles, depth, directive) {
    if (node.tag) {
      ...
      const children = node.children;
      if (!children || children.length === 0) {
        return transpilerShared.indent(`<${node.tag} ${directive}${attrs} />`, depth);
      }
      const childrenHTML = children.map((child) => this.generateNode(child, styles, depth + 1)).join("\n");
      return [
        transpilerShared.indent(`<${node.tag} ${directive}${attrs}>`, depth),
        childrenHTML,
        transpilerShared.indent(`</${node.tag}>`, depth)
      ].join("\n");
    }
    const content = this.generateNode(node, styles, depth + 1);
    return [
      transpilerShared.indent(`<template ${directive}>`, depth),
      content,
      transpilerShared.indent("</template>", depth)
    ].join("\n");
  }
  ...
  generateLoop(node, styles, depth) {
    ...
    if (children.length === 1 && children[0]?.tag) {
      ...
      if (!grandchildren || grandchildren.length === 0) {
        return transpilerShared.indent(`<${child.tag}${vForAttr}${childAttrs} />`, depth);
      }
      const childrenHTML2 = grandchildren.map((gc) => this.generateNode(gc, styles, depth + 1)).join("\n");
      return [
        transpilerShared.indent(`<${child.tag}${vForAttr}${childAttrs}>`, depth),
        childrenHTML2,
        transpilerShared.indent(`</${child.tag}>`, depth)
      ].join("\n");
    }
    const childrenHTML = children.map((child) => this.generateNode(child, styles, depth + 1)).join("\n");
    return [
      transpilerShared.indent(`<template v-for="${itemVar} in ${collection}"${keyAttr}>`, depth),
      childrenHTML,
      transpilerShared.indent("</template>", depth)
    ].join("\n");
  }
  ...
  generateComponentRef(node, depth) {
    ...
    const propsStr = Object.entries(props).map(([key, value]) => {
      if (transpilerShared.isPureExpression(value)) {
        return `:${key}="${this.convertExprToVue(transpilerShared.extractExpression(value))}"`;
      }
      return `${key}="${value}"`;
    }).join(" ");
  ...
  buildVueAttributes(node) {
    ...
    if (node.attributes) {
      for (const [key, value] of Object.entries(node.attributes)) {
        ...
        if (transpilerShared.isPureExpression(value)) {
          parts.push(`:${key}="${this.convertExprToVue(transpilerShared.extractExpression(value))}"`);
        } else {
    ...
    if (node.style) {
      const entries = Object.entries(node.style).map(([prop, val]) => {
        if (transpilerShared.isPureExpression(val)) {
          return `${prop}: ${this.convertExprToVue(transpilerShared.extractExpression(val))}`;
        }
        return `${prop}: '${val}'`;
      }).join(", ");
  ...
  propToTSType(type, values) {
    ...
    return transpilerShared.irTypeToTSBase(type);
  }
};

exports.VueTranspiler = VueTranspiler;
